    ...
    },
    {
      ...
      cell: (row) => (
        <span className="font-extrabold text-slate-800 dark:text-white">
          {row.type === 'percentage' ? `${row.discountValue}% Off` : `${currencySymbol}${row.discountValue} Off`}
        </span>
      )
    },
    {
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="form-label text-xs">Minimum Order Amount ({currencySymbol})</label>
              <input
                type="number"
                min="0"
                step="0.01"
